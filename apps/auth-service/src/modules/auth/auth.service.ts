...
export class AuthService extends BaseService implements OnModuleInit, OnModuleDestroy {
  ...
  private notificationGrpcService!: Record<
    string,
  ...
  async login(body: LoginDto): Promise<LoginResponseDto> {
    ...
    const user = await this.msResponse(
      this.callUserGrpc(UserMessagePattern.GET_USER, { email }),
    );
  ...
  async signUp(body: SignUpDto): Promise<SignUpResponseDto> {
    ...
    const newUser = await this.msResponse(
      this.callUserGrpc(UserMessagePattern.CREATE_USER, userData),
    );
  ...
  async sendResetPasswordLink(body: ForgotPasswordDto): Promise<SuccessResponseDto> {
    ...
    await this.msResponse(
      this.callNotificationGrpc(NotificationMessagePattern.FORGOT_PASSWORD, {
        email,
        name: user.fullName,
        resetPasswordUrl,
      }),
  ...
  async resetPassword(body: ResetPasswordDto): Promise<SuccessResponseDto> {
    ...
    await this.msResponse(this.callUserGrpc(UserMessagePattern.UPDATE_USER, userData));
  ...
  async changePassword(
    ...
    const user = await this.msResponse(
      this.callUserGrpc(UserMessagePattern.GET_USER, {
  ...
  private callUserGrpc(
    methodName: UserMessagePattern,
    data: unknown,
  ): Observable<unknown> {
    return this.callGrpcMethod(this.userGrpcService, methodName, data);
  }

  private callNotificationGrpc(
    methodName: NotificationMessagePattern,
    data: unknown,
  ): Observable<unknown> {
    return this.callGrpcMethod(this.notificationGrpcService, methodName, data);
  }

  private callGrpcMethod(
    grpcService: Record<string, (data: unknown) => Observable<unknown>>,
    methodName: string,
    data: unknown,
  ): Observable<unknown> {
    const camelCaseMethodName = methodName[0].toLowerCase() + methodName.slice(1);
    const grpcMethod = grpcService[methodName] ?? grpcService[camelCaseMethodName];

    if (!grpcMethod) {
      throw new Error(`gRPC method not found: ${methodName}`);
    }

    return grpcMethod.call(grpcService, data);
  }
}
