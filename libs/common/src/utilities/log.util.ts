...
interface LogBootstrapOptions {
  ...
  appPort: number;
  grpcListener?: Record<string, any>;
}
...
export function logBootstrapInfo(
  ...
  const { grpcListener, nodeEnv, logger, appPort } = logOptions;
  ...
  if (grpcListener) {
    logger.log({
      message: `gRPC Microservice Listener is ready on ${chalk.blue(
        `${grpcListener?.url || 'Unknown'}`,
      )}`,
